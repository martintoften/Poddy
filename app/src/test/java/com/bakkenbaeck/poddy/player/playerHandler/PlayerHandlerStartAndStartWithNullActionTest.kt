...
class PlayerHandlerStartAndStartWithNullActionTest : KoinTest {
    ...
    @Test
    fun `add start and start with null episode action`() = runBlocking {
        playerHandler.init()
        playerHandler.handlePlayerAction(ACTION_START, serialMockEpisode)
        playerHandler.handlePlayerAction(ACTION_START, null)
